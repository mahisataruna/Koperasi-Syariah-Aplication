...
<body>
  ...
  <hr>
  <!-- Ambil data anggota -->
  <?php foreach ($loadAgtAngsur as $la) :?>
  <div class="row mb-4">
    <div class="col-sm-6">
  ...
  <tbody>
    <?php $i=1; ?>
    <?php foreach ($fakturAng as $fa) : 
    {
      $nominal1=number_format($fa->jumlah_angsuran,0,",",".")
      ?>
      <tr id="example1_filter">
        <td scope="row" width="3%" style="text-align: center;"><?= $i; ?></td>                  
        <td width="3%" style="text-align: center;"><?= $fa->id_angsuran; ?></td>
        <td width="5%" style="text-align: center;"><?= $fa->id_pinjaman; ?></td>
        <td width="5%" style="text-align: center;"><?= $fa->angsuran_ke; ?></td>
        <td width="10%" style="text-align: center;">Rp. <?= $nominal1; ?></td>
        <td width="10%" style="text-align: center;"><?= $fa->tanggal; ?></td>
      </tr>
      <?php $i++;?>
      <?php } ?>
      <?php endforeach; ?>
